    ...
    public function processMenus($paths)
    {
        foreach ($paths as $path) {
            if (!$this->isResourcePath($path)) {
                ...
                if ($this->isPageFile($path) && $this->hasMenu($page)) {
        ...
        try {
            ...
            if (empty($path)) {
                ...
                if ($this->fs->has($path)) {
                    ...
                    if ($this->fs->get($this->draftBaseUrl() . $path) instanceof
                        Directory
                    ) {
                        $path .= '/' . $defaultIndex;
                    }
    ...
    public function updateFromSource()
    {
        ...
        foreach ($paths as $path) {
            ...
            if (!$this->fs->has($draftPath)) {
                ...
                if ($this->isPageFile($path) && !isset($this->metadata['pages'][$path])) {
                    $page = new Page($this->fs->read($draftPath));
                    $id = Utils::id();
                    $this->metadata['pages'][$path] = [
                        // Set page id
                ...
                } elseif (!$this->isPageFile($path) && isset($this->metadata['pages'][$path])) {
